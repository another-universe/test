...
@push('vue_templates')
<script type="text/x-template" id="__wrapper">
<edit-form text="{{ $quote->getText() }}" />
</script>

<script type="text/x-template" id="_editQuoteForm">
<form
ref="form"
role="form"
data-action-url="{{ lroute('quotes.update', compact('quote')) }}"
novalidate
>
<div class="mb-3 row">
<label class="col-3 col-form-label" for="text">
{{ __('quote/edit_page.form.text') }}
</label>
...
<template v-else>
{{ __('quote/edit_page.form.submit') }}
</template>
</button>
</div>
</div>
